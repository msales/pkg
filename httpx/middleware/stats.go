...
func WithRequestStats(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		stats.Inc(r.Context(), "request.start", 1, 1.0, map[string]string{
			"method": r.Method,
			"path":   r.URL.Path,
		})
		...
		stats.Inc(r.Context(), "request.complete", 1, 1.0, map[string]string{
...
func WithResponseTime(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t := stats.Time(r.Context(), "response.time", 1.0, nil)
		defer t.Done()
